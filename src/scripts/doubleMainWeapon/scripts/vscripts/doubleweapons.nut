...
if (Entities.FindByName(null, "doubleweapon_timer") == null) {
	SpawnEntityFromTable("logic_timer", {
		targetname = "doubleweapon_timer",
		vscripts = "doubleweapons",
		RefireTime = 0.15,
		OnTimer = "!caller,runscriptcode,PlayerRunCmds()"
	});
}
...
function DeleteNickArray(ent) //删除数组的主函数
{
	...
	if (n != null) {
		::Nicks.remove(n);::Weapons.remove(n);::fAmmo.remove(n);::sAmmo.remove(n);::Times_.remove(n);::Upgard.remove(n);::uAmmo.remove(n);
		Msg("deleted");
	}
...
function CreateNickArray() //创建数组的主函数
{
	...
	while (ent = Entities.FindByClassname(ent, "player")) //遍历全部玩家
	{
		...
		{ //创建数组
			...
			{
				::Nicks.append(ent);::Weapons.append("weapon_none");::fAmmo.append(-1);::sAmmo.append(-1);::Times_.append(0);::Upgard.append(0);::uAmmo.append(-1);
				//Msg("1");
...
function SaveMyWeapon(ent, num, tpw = null, tpf = null, tps = null, tpu = null, tpum = null, tpr = false) {
	...
	if (Times_[num] == 0)
		::Times_[num] = 1;
	if (tpr == false) {
		::Weapons[num] = nowWeapon;::fAmmo[num] = (NetProps.GetPropInt(GetPlayerWeapon(ent, 0), "m_iClip1")).tointeger();::sAmmo[num] = (NetProps.GetPropIntArray(ent, "m_iAmmo", NetProps.GetPropInt(GetPlayerWeapon(ent, 0), "m_iPrimaryAmmoType"))).tointeger();::Upgard[num] = (NetProps.GetPropInt(GetPlayerWeapon(ent, 0), "m_upgradeBitVec")).tointeger();
		...
		::Weapons[num] = tpw;::fAmmo[num] = tpf;::sAmmo[num] = tps;::Upgard[num] = tpu;::uAmmo[num] = tpum;
	}
