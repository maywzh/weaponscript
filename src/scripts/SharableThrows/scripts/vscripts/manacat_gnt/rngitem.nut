...
::manacat_rng_item <- {
	...
	function OnGameEvent_spawner_give_item(params){
		...
		/*if(params.item == "weapon_rifle_m60" || params.item == "weapon_grenade_launcher"){
			...
			if("rngskin" in rng)printl(rng.rngskin);
	...
	function OnGameEvent_weapon_fire(params){
		...
		if(weapon == "molotov" || weapon == "pipe_bomb" || weapon == "vomitjar")
			EntFire( "worldspawn", "RunScriptCode", "g_ModeScript.manacat_rng_item.throwsSkin("+params.userid+")", 0.13 );
	}

	function throwsSkin(userid){
		local player = GetPlayerFromUserID(userid);
		printl(player.GetPlayerName());
		player.ValidateScriptScope();
		local scrScope = player.GetScriptScope();
		local ent = null;	local chk = true;
		while (ent = Entities.FindByClassname(ent, "*_projectile")){
			ent.ValidateScriptScope();
			local throwScope = ent.GetScriptScope();
			if(ent.IsValid() && !("rngchk" in throwScope) && NetProps.GetPropEntity( ent, "m_hThrower" ) == player){
				printl(scrScope.throwSkin);
				throwScope.rngchk <- true;	chk = false;
				NetProps.SetPropInt( ent, "m_nSkin", scrScope.throwSkin );break;
			}
		}
		if(chk)EntFire( "worldspawn", "RunScriptCode", "g_ModeScript.manacat_rng_item.throwsSkin("+userid+")", 0.033 );
	}
	...
	function foot_locker_rng(){
		...
		for(local i = 0; i < 3; i++)
			while(entity = Entities.FindByClassname(entity, "prop_dynamic")){
				...
				if(model in dummy)::manacat_rng_item.SetSkin(entity, dummy[entity.GetModelName()], true);
			}
	}
	...
	function gamemode(){
		...
		if(mp_gamemodebase == "coop")return "coop";
	...
	function ResetSkinSupplies(){
		if(!::manacat_rng_item.startflag || ("itemSpawner" in ::MANACAT && !::MANACAT.itemSpawner.check)){
			EntFire( "worldspawn", "RunScriptCode", "g_ModeScript.manacat_rng_item.ResetSkinSupplies()", 0.1 );
			return;
		}
		...
		EntFire( "worldspawn", "RunScriptCode", "g_ModeScript.manacat_rng_item.ResetSkinProcess()", 0.1 );
		SaveTable("rngitemspawn", ::manacat_rng_item.sessionData);
